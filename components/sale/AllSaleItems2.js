...
export default function AllSaleItems2({setRows, rows, totalPrice, setTotalPrice}) {
    const { data } = useGlobalState();
    const [items, setItems] = useState(data?.data.items.map(item => ({ 
        ...item, 
        quantity: 0, 
        taxAsked: false, 
        taxPercentage: 0 
        ...
        if (existingRowIndex >= 0) {
            const updatedRows = rows.map((row, i) => 
                i === existingRowIndex
                    ? {
                        ...row,
                        qty: row.qty + 1,  
                        amount: (row.qty + 1) * row.price_per_unit * (1 + selectedItem.taxPercentage / 100)
                    }
                    : row
            );
            setRows(updatedRows);
            ...
            setRows(prevRows => [
                ...prevRows,
                {
                    ...
                    discount: "",
                    profit: 0,
                    amount: (selectedItem.atPrice || 0) * (1 + selectedItem.taxPercentage / 100),
                    tax: selectedItem.taxPercentage,
    ...
    const handleTaxSubmit = () => {
        const taxValue = parseFloat(currentTaxPercentage);
        setItems(prevItems => prevItems.map((item, index) => 
            index === currentItemIndex 
                ? { ...item, taxAsked: true, taxPercentage: taxValue } 
                : item
        ));
        setModalVisible(false);
        addRow(currentItemIndex);
    };
    const handleSubmit = () => {
        const filteredRows = rows.filter(row => row.qty > 0);
        let total_price = 0;
        filteredRows.forEach(row => {
            total_price += row.price_per_unit * row.qty * (1 + row.taxPercentage / 100);
        });
        setTotalPrice(total_price);
        setRows(filteredRows);
        console.log("new test",filteredRows)
    ...
    textAlign: 'center',
            ...
            <TaxModal modalVisible={modalVisible} setModalVisible={setModalVisible} currentTaxPercentage={currentTaxPercentage} setCurrentTaxPercentage={setCurrentTaxPercentage} currentItemIndex={currentItemIndex} items={items} handleTaxSubmit={handleTaxSubmit} /> 
        </>
